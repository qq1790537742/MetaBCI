    ...
    if pdim == "ssvep":
        ...
        def co_stim():
            ...
            while not event.getKeys(["space"]):
                # if get
                sf = frame_num % VSObject.refresh_rate
                VSObject.flash_stimuli_co[sf].draw()
                win.flip()
                frame_num += 1
            book_id = 1
            return book_id

        def re_stim(book_id, page, fun):
            # initialise index position
            id = int(trial["id"])
            position = VSObject.stim_pos_re[id] + np.array([0, VSObject.stim_width_re / 2])
            VSObject.index_stimuli.setPos(position)
            ...
            if book_id != None:
                while not event.getKeys(["space"]):
                    sf = frame_num % VSObject.refresh_rate
                    VSObject.flash_stimuli_re[sf].draw()
                    VSObject.pic_read_set[book_id][page].draw()
                    for text_stimulus in VSObject.text_stimuli:
                        text_stimulus.draw()
                    win.flip()
                    frame_num += 1

        def wait_stim(book_id, page, fun):
            # initialise index position
            id = int(trial["id"])
            position = VSObject.stim_pos_re[id] + np.array([0, VSObject.stim_width_re / 2])
            VSObject.index_stimuli.setPos(position)

...
from AlgorithmSystem.AlgorithmImplement.SSVEP.function import EstimateSTEqualizer, firfilter_matrix, iterate_qr, \
    update_inner_product
        ...
        if not self.cal_flag:
            ...
            for index in range(self.detect_win_num - self.check_win_num):
                if np.var(result_list[index:index + self.check_win_num]) == 0 and \
                        len(np.where(np.asarray(judge_list[index:index + self.check_win_num]) > self.threshold)[
                                0]) >= 2:
...
class MultilayerWindowClassifier(object):
    ...
    def add_data(self, new_data):
        ...
        update_inner_product(self.QxQy_inner_product, self.current_index, x_G1, x_G2, self.template_G1,
                             self.template_G2)
    ...
    def get_result(self):
        ...
        for i in range(max_window):
            judge1 = np.log(np.sum(np.exp(-0.5 * (noise_energy[:, i] - min(noise_energy[:, i]))))) <= 10 ** -int(
                self.threshold[0])

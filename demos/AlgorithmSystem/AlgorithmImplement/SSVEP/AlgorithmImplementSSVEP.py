...
from AlgorithmSystem.AlgorithmImplement.SSVEP.algorithm import MultilayerWindowClassifierParameters, \
    MultilayerWindowClassifier, FastCCA
...
class AlgorithmImplementSSVEP(AlgorithmInterface):
    ...
    def __init__(self):
        super().__init__()
        self.stimulation_frequency_set = np.linspace(8.0, 15.8, 40)
        self.channel_no = 8
        ...
        self.threshold_ste = np.array([10, 8.75, 0.015])
        self.raw_detection_layer = np.array([5, 4])

        self.equalizer_order = [4, 7]
        self.equalizer_update_interval = 250 * 10
        self.equalizer_estimate_length = 250 * 40
    ...
    def run(self):
        ...
        while True:
            ...
            if block_end:
                # if sum(self.report_record.values()) <= 25:
                #     self.ste_classifier.threshold -= [1, 1, 0.004]
                # self.__print_result()
                self.result_record = []
                self.false_result = []
                ...
                # self.cca_flag = True
                self.data_cache = None
                self.data_reader.clear()
                # if len(self.all_result) % 4 == 0:

                # print('reset threshold. old threshold: {}'.format(self.ste_classifier.threshold))
                # self.ste_classifier.threshold = np.copy(self.threshold_ste)
                # self.detection_layer = self.raw_detection_layer
                continue
            ...
            if result > 0:
                # self.detect_record[result] = self.detect_record.get(result, 0) + 1
                # self.result_record.append(result)
                # self.report_record[result] = self.report_record.get(result, 0) + 1
    ...
    def __print_result(self):
        print('被试 {} block {}'.format(len(self.all_result) // 4 + 1, len(self.all_result) % 4 + 1))
        print('检出{}({})个结果：'.format(sum(self.detect_record.values()), len(self.detect_record)), self.detect_record)
        print('汇报{}({})个结果：'.format(sum(self.report_record.values()), len(self.detect_record)), self.report_record)
        self.all_result.append(self.report_record)
        self.report_record = {}
        self.detect_record = {}
